    ...
    <div className="flex flex-col gap-8 sm:gap-10 max-w-3xl w-full">
      ...
      <ScrollReveal delay={0.1}>
        <div className="card-base flex flex-col gap-6">
          ...
          <ScrollRevealGroup className="grid gap-3 sm:grid-cols-3" stagger={0.08}>
            {support.stats.map((stat) => {
              const Icon = STAT_ICONS[stat.icon as keyof typeof STAT_ICONS] ?? Star;
              return (
                <ScrollRevealItem key={stat.label} variant="scale">
                  <div className="flex items-start gap-2 rounded-lg border border-border-dim bg-page/50 p-3 text-sm text-text-secondary h-full">
