...
export async function proxy(request: NextRequest) {
    ...
    const isHeroRoute = pathname === '/' || pathname === '/thank-you'
    const isPublicArticleRoute = pathname.startsWith('/article/')
    const isStaticAsset = /\.(?:png|jpe?g|gif|svg|webp|ico|woff2?|ttf|eot)$/i.test(pathname)
    const isPublicRoute = pathname.startsWith('/_next') || pathname.startsWith('/api') || pathname === '/favicon.ico' || isStaticAsset || isHeroRoute || isPublicArticleRoute
